    ...
    <div class="p-5">
      <form onsubmit="SecurityAuth.changePassword(event)" class="space-y-4 max-w-md">
        <div>
          ...
          <input type="password" id="current-pass" required class="w-full px-4 py-2.5 rounded-lg border border-slate-200 focus:border-orange-500">
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Nueva contraseña</label>
          <input type="password" id="new-pass" required class="w-full px-4 py-2.5 rounded-lg border border-slate-200 focus:border-orange-500">
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Confirmar contraseña</label>
          <input type="password" id="confirm-pass" required class="w-full px-4 py-2.5 rounded-lg border border-slate-200 focus:border-orange-500">
        </div>
    ...
    <div class="p-5 space-y-4">
      <div class="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
        <div>
          ...
          <p class="text-sm text-slate-500">Bloquear después de 5 intentos fallidos (local)</p>
        </div>
        <label class="relative inline-flex items-center cursor-pointer">
          <input type="checkbox" ${security.lockOnFailure ? 'checked' : ''} onchange="SecurityAuth.toggleLock(this.checked)" class="sr-only peer">
          <div class="w-11 h-6 bg-slate-200 peer-focus:ring-4 peer-focus:ring-orange-100 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-['']
            after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-orange-500"></div>
        </label>
      </div>
...
`,
      ...
      onConfirm: () => {
        ...
        Store.security.addLog('2FA activado', 'auth');
        Components.toast('2FA activado correctamente', 'success');
        Router.navigate('security', {}, { push: false });
      }
    });
  },
  ...
  disable2FA() {
    Components.modal({
      ...
      onConfirm: () => {
        ...
        Store.security.addLog('2FA desactivado', 'auth');
...
`,
  ...
  async changePassword(e) {
    ...
    if (idx === -1) {
      Components.toast('Usuario no encontrado', 'error');
      return;
    }
  ...
  toggleLock(enabled) {
    ...
    Store.security.addLog(enabled ? 'Bloqueo por intentos activado' : 'Bloqueo por intentos desactivado', 'config');
  ...
  toggleEncryption(enabled) {
    ...
    Store.security.addLog(enabled ? 'Encriptación activada' : 'Encriptación desactivada', 'config');
    Components.toast('Configuración actualizada', 'success');
  }
};
