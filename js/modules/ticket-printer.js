window.TicketPrinter = {
  ...
  currency(amount) {
    ...
    const symbols = { USD:'$', MXN:'$', EUR:'€', COP:'$', CUP:'$', MLC:'$', ARS:'$', CLP:'$', PEN:'S/', VES:'Bs', BRL:'R$', CAD:'$' };
  ...
  buildTicketHTML(sale, { autoPrint=true, widthOverride=null } = {}) {
    ...
    const width = widthOverride || cfg.width;
...
<tr><td class="wrap">${name}</td><td class="r nowrap">${qty}</td><td class="r nowrap">${this.currency(price)}</td><td class="r nowrap">${this.currency(lineTotal)}</td></tr>
` : `
<tr><td class="wrap">${name}</td><td class="r nowrap">${qty}</td><td class="r nowrap">${this.currency(lineTotal)}</td></tr>
`;
      }).join('');
      itemsHTML = `<table><thead>${head}</thead><tbody>${rows}</tbody></table>`;
    }
...
<style>
@page{margin:0}
body{margin:0;background:#fff;font-family:ui-monospace,Menlo,Consolas,monospace;font-size:${baseFont}px;line-height:1.25}
.ticket{width:${pageWidthCss};padding:10px;box-sizing:border-box}
...
if(AUTO){setTimeout(()=>{window.focus();window.print()},250);}
</script>
</body></html>`;
  },
  ...
  _openHtmlAsBlob(html) {
    if (typeof html !== 'string' || !html.trim().toLowerCase().startsWith('<!doctype html')) {
      console.error('Ticket HTML inválido:', String(html).slice(0, 120));
      Components.toast('Error: Ticket HTML inválido. Revisa ticket-printer.js y caché.', 'error', 6000);
      return null;
    }
    ...
    if (!w) {
      ...
      Components.toast('Permite popups para imprimir', 'error');
      return null;
    }
  ...
  printTest() {
    const dummy = {
      ...
      items: [
        { name: 'Producto largo para probar que NO se corte en 58mm', qty: 2, price: 10 },
