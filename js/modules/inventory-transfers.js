...
<div class="space-y-4">
  <div class="flex justify-between items-center">
    ...
    <button onclick="InventoryTransfers.openModal()"
      ...
      transition btn-press flex items-center gap-2">
      ...
      <table class="w-full">
        <thead class="bg-slate-50">
          <tr class="text-left text-sm text-slate-600">
            ...
            <th class="px-4 py-3 font-semibold">Stock movido</th>
...
`;
  ...
  openModal() {
    ...
    Components.modal({
      title: 'Nueva Transferencia (Ubicación)',
      content: `
<form id="transfer-form" class="space-y-4">
  <div>
    ...
    <p class="text-xs text-slate-500 mt-1">
      Nota: con el modelo actual, esta transferencia mueve el producto (ubicación) con todo su stock.
    </p>
  </div>
...
`,
      ...
      onConfirm: () => {
        ...
        if (!productId) {
          Components.toast('Selecciona un producto', 'error');
          return;
        }
        ...
        // Validación clave: el origen debe ser el almacén actual del producto
        if ((product.warehouseId || 1) !== fromWarehouse) {
          Components.toast('El almacén origen no coincide con la ubicación actual del producto', 'error');
          return;
        }

        // Con stock global, se mueve TODO el stock (no parcial)
