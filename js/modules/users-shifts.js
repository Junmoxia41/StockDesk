...
<div class="space-y-6">
  ...
  <div class="bg-green-50 rounded-xl p-6 border border-green-200">
    <div class="flex items-center justify-between mb-4">
      ...
      <button onclick="UsersShifts.closeShift(${currentShift.id})"
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div class="bg-white rounded-lg p-3 text-center">
        ...
        <p class="font-semibold text-green-600">$${Number(currentShift.openingCash || 0).toFixed(2)}</p>
      </div>
      <div class="bg-white rounded-lg p-3 text-center">
        <p class="text-xs text-slate-500">Ventas</p>
        <p class="font-semibold text-orange-600">$${Number(currentShift.sales || 0).toFixed(2)}</p>
      </div>
    </div>
  </div>
  ` : `
      ...
      <table class="w-full">
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr class="table-row-hover">
            ...
            <td class="px-4 py-3 text-sm">$${Number(s.openingCash || 0).toFixed(2)}</td>
            <td class="px-4 py-3 text-sm font-semibold text-green-600">$${Number(s.sales || 0).toFixed(2)}</td>
            <td class="px-4 py-3 text-sm">$${Number(s.closingCash || 0).toFixed(2)}</td>
          </tr>
          `).join('')}
        </tbody>
      </table>
...
`;
  ...
  renderRegisters() {
    const registers = Store.get(Store.KEYS.REGISTERS) || [{ id: 1, name: 'Caja Principal', status: 'active' }];
    return `
<div class="space-y-4">
    ...
    ${registers.map(reg => `
      <div class="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
        ...
        <div class="flex gap-2">
          <button onclick="UsersShifts.editRegister(${reg.id})" class="flex-1 py-2 text-sm bg-slate-100 hover:bg-slate-200 rounded-lg">
            Editar
          </button>
        </div>
      </div>
...
`;
  ...
  openShift() {
    const registers = Store.get(Store.KEYS.REGISTERS) || [{ id: 1, name: 'Caja Principal' }];
    const user = Store.get(Store.KEYS.USER)?.username || 'Usuario';

    const shifts = Store.get(Store.KEYS.SHIFTS) || [];
    if (shifts.some(s => s.status === 'open')) {
      Components.toast('Ya existe un turno abierto', 'warning');
      return;
    }
...
<form class="space-y-4">
  ...
  <div>
    ...
    <input type="number" id="shift-opening" step="0.01" value="0"
...
`,
      confirmText: 'Abrir',
      onConfirm: () => {
        const shiftsNow = Store.get(Store.KEYS.SHIFTS) || [];
        shiftsNow.push({
          id: Date.now(),
          cashier: user,
        ...
        Store.set(Store.KEYS.SHIFTS, shiftsNow);
        if (window.addAuditLog) addAuditLog('Crear', 'Turnos', 'Turno abierto');
...
<form class="space-y-4">
  <div>
    ...
    <input type="number" id="shift-closing" step="0.01"
      class="w-full px-4 py-2.5 rounded-lg border border-slate-200">
  </div>
  ...
  <div>
    ...
    <textarea id="shift-notes" rows="2"
      class="w-full px-4 py-2.5 rounded-lg border border-slate-200"></textarea>
  </div>
</form>
`,
      confirmText: 'Cerrar',
      onConfirm: () => {
        ...
        if (idx !== -1) {
          ...
          shifts[idx].notes = document.getElementById('shift-notes').value || '';
          shifts[idx].status = 'closed';
          Store.set(Store.KEYS.SHIFTS, shifts);
          if (window.addAuditLog) addAuditLog('Editar', 'Turnos', 'Turno cerrado');
...
`,
      confirmText: 'Crear',
      onConfirm: () => {
        const registers = Store.get(Store.KEYS.REGISTERS) || [];
        registers.push({
          id: Date.now(),
        ...
        Store.set(Store.KEYS.REGISTERS, registers);
