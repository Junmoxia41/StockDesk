...
const LayoutComponents = {
  ...
  // Helpers RBAC (local, basado en roles guardados)
  _getRolePermissions(roleName) {
    let roles = Store.get(Store.KEYS.ROLES);
    if (!roles || roles.length === 0) {
      roles = [
        { name: 'Administrador', permissions: ['all'] },
        { name: 'Gerente', permissions: ['products', 'inventory', 'sales', 'reports', 'users', 'settings', 'finance', 'security', 'suppliers', 'notifications', 'dashboards'] },
        { name: 'Cajero', permissions: ['sales', 'reports', 'notifications'] },
        { name: 'Inventario', permissions: ['products.view', 'products.edit', 'inventory', 'reports'] }
      ];
    }
  ...
  _expandPermissions(perms = []) {
    const set = new Set(perms);
    if (set.has('all')) return ['all'];

    // compat: products => CRUD completo + view
    if (set.has('products')) {
    ...
    // compat: sales => descuento
    if (set.has('sales')) set.add('sales.discount');

    return Array.from(set);
  },

  _hasAnyPermission(userPerms, required = []) {
    const perms = this._expandPermissions(userPerms || []);
  ...
  sidebar(activeRoute = 'dashboard') {
    ...
    const linksAll = [
      { id: 'dashboard', icon: 'home', label: 'Inicio', perms: ['sales', 'reports', 'products.view', 'inventory', 'finance'] },
      { id: 'sales', icon: 'cart', label: 'Ventas', perms: ['sales'] },
      { id: 'reports', icon: 'chart', label: 'Reportes', perms: ['reports', 'sales'] },
      { id: 'products', icon: 'package', label: 'Productos', perms: ['products.view', 'products'] },
      { id: 'inventory', icon: 'warehouse', label: 'Inventario', perms: ['inventory'] },
      { id: 'suppliers', icon: 'users', label: 'Proveedores', perms: ['suppliers'] },
      { id: 'finance', icon: 'dollar', label: 'Finanzas', perms: ['finance'] },
      { id: 'users', icon: 'users', label: 'Usuarios', perms: ['users'] },
      { id: 'notifications', icon: 'info', label: 'Notificaciones', perms: ['notifications'] },
      { id: 'customization', icon: 'star', label: 'Personalización', perms: ['settings'] },
      { id: 'security', icon: 'lock', label: 'Seguridad', perms: ['security'] },
      { id: 'settings', icon: 'settings', label: 'Ajustes', perms: ['settings'] },
      { id: 'dashboards', icon: 'chart', label: 'Dashboards', perms: ['dashboards', 'reports', 'sales', 'finance', 'inventory'] },
      { id: 'catalog', icon: 'grid', label: 'Catálogo', perms: ['all'] }, // solo admin/gerente normalmente
      { id: 'guide', icon: 'brain', label: 'Guía', perms: [] } // visible para todos los logueados
    ];

    const links = linksAll.filter(l => this._hasAnyPermission(perms, l.perms));
...
<div id="mobile-menu" class="mobile-menu fixed inset-0 z-50 bg-white">
  ...
  <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-slate-100">
    <button onclick="LayoutComponents.logout()"
      class="flex items-center gap-3 px-4 py-3 rounded-lg text-slate-500 hover:bg-red-50 hover:text-red-600 transition w-full">
...
<aside class="sidebar-desktop w-16 hover:w-52 bg-white border-r border-slate-200 h-screen fixed left-0 top-0 flex flex-col transition-all duration-300 group z-30 overflow-hidden">
      ...
      ${links.map(link => `
        <li>
          <a href="#" onclick="Router.navigate('${link.id}'); return false;"
            class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 hover:bg-orange-50 hover:text-orange-600 transition ${activeRoute === link.id ? 'active' : ''}"
            title="${link.label}">
  ...
  <div class="p-2 border-t border-slate-100">
    <button onclick="LayoutComponents.logout()"
      class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-500 hover:bg-red-50 hover:text-red-600 transition w-full"
      title="Salir">
      ${Components.icons.logout}
