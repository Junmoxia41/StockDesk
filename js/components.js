...
const Components = {
    ...
    modal(options) {
        const { title, content, onConfirm, onCancel, confirmText = 'Confirmar', cancelText = 'Cancelar', type = 'default' } = options;
        ...
        modal.innerHTML = `
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md animate-scale-in max-h-[90vh] overflow-y-auto" onclick="event.stopPropagation()">
                <div class="p-6">
                ...
                <div class="flex gap-3 p-4 bg-slate-50 rounded-b-2xl sticky bottom-0">
                    <button id="modal-cancel" class="flex-1 px-4 py-2.5 rounded-lg border border-slate-200 text-slate-700 font-medium hover:bg-slate-100 transition btn-press">${cancelText}</button>
                    <button id="modal-confirm" class="flex-1 px-4 py-2.5 rounded-lg ${type === 'danger' ? 'bg-red-500 hover:bg-red-600' : 'bg-orange-500 hover:bg-orange-600'} text-white font-medium transition btn-press">${confirmText}</button>
                </div>
            </div>
        `;
        ...
        confirmBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            // Ejecutar onConfirm y pasar función para cerrar
            if (onConfirm) {
                const result = onConfirm(closeModal);
                // Si retorna false explícitamente, no cerrar
                if (result !== false) {
                    closeModal();
                }
            } else {
    ...
    statCard(title, value, icon, color = 'orange') {
        const colors = { orange: 'bg-orange-100 text-orange-600', green: 'bg-green-100 text-green-600', blue: 'bg-blue-100 text-blue-600', purple: 'bg-purple-100 text-purple-600' };
        return `
