        ...
        <tr class="table-row-hover">
          <td class="px-4 md:px-6 py-3 md:py-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <span class="font-medium text-slate-900 text-sm md:text-base block">${Sanitize.escapeHtml(p.name)}</span>
                ${p.barcode ? `<span class="text-xs text-slate-400">${Sanitize.escapeHtml(p.barcode)}</span>` : ''}
              </div>
            </div>
          </td>
          <td class="px-4 md:px-6 py-3 md:py-4">
            <span class="px-2 md:px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-xs md:text-sm">${Sanitize.escapeHtml(p.category)}</span>
          </td>
  ...
  <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
    <div class="flex items-start justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h4 class="font-medium text-slate-900">${Sanitize.escapeHtml(p.name)}</h4>
          <span class="text-xs px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full">${Sanitize.escapeHtml(p.category)}</span>
        </div>
      </div>
  ...
  <div class="grid grid-cols-2 gap-4">
    ...
    <div>
      ...
      <select id="prod-warehouse" class="w-full px-4 py-2.5 rounded-lg border border-slate-200 focus:border-orange-500">
        ${warehouses.map(w => `<option value="${w.id}" ${product?.warehouseId === w.id ? 'selected' : ''}>${Sanitize.escapeHtml(w.name)}</option>`).join('')}
      </select>
    </div>
  </div>
</form>
`;
  ...
  deleteProduct(id) {
    ...
    Components.modal({
      ...
      content: `<p>¿Eliminar "<strong>${Sanitize.escapeHtml(product.name)}</strong>"? Esta acción no se puede deshacer.</p>`,
