...
<div class="animate-fade-in">
  ...
  <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-3 md:p-4 mb-4 md:mb-6">
    <div class="flex flex-wrap items-center gap-3 md:gap-4">
      ...
      <button onclick="ReportsPage.exportCSV()" class="ml-auto px-3 py-2 text-sm bg-slate-100 hover:bg-slate-200 rounded-lg transition flex items-center gap-2">
        ${Components.icons.clipboard} Exportar CSV
      </button>
    </div>
  </div>
        ...
        <div class="space-y-2 md:space-y-3">
          ...
          <div class="flex items-center justify-between p-2 md:p-3 bg-slate-50 rounded-lg">
            <div>
              ...
              <p class="text-xs text-slate-400">${(sale.items || []).reduce((acc, item) => acc + (item.qty || 0), 0)} items</p>
            </div>
            ...
            <div class="flex items-center gap-2">
              ...
              <button onclick="ReportsPage.viewSale(${sale.id})"
                class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg" title="Ver detalle">
                ${Components.icons.info}
              </button>

              <button onclick="ReportsPage.printSale(${sale.id})"
                class="p-2 text-slate-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg" title="Imprimir ticket">
...
`;
  ...
  getDailySales(sales) {
    ...
    sales.forEach(sale => {
      ...
      daily[date] = (daily[date] || 0) + (sale.total || 0);
    });
    return daily;
  },
  ...
  updateDateFrom(value) { this.dateFrom = value; Router.navigate('reports', {}, { push: false }); },
  updateDateTo(value) { this.dateTo = value; Router.navigate('reports', {}, { push: false }); },
  ...
  viewSale(id) {
    ...
    Components.modal({
      title: `Venta #${String(sale.id).slice(-6)}`,
      content: `
<div class="space-y-3">
  <div class="text-sm text-slate-500">
    <div><strong>Fecha:</strong> ${new Date(sale.date).toLocaleString('es')}</div>
    <div><strong>Cliente:</strong> ${sale.customer || 'Público General'}</div>
  </div>

  <div class="border-t border-slate-100 pt-3">
    <p class="text-sm font-semibold text-slate-900 mb-2">Items</p>
    <div class="max-h-44 overflow-y-auto space-y-2">
      ${(sale.items || []).map(i => `
        <div class="flex justify-between text-sm bg-slate-50 px-3 py-2 rounded-lg">
          <span>${i.name} x${i.qty}</span>
          <span class="font-semibold">$${(i.price * i.qty).toFixed(2)}</span>
        </div>
      `).join('')}
    </div>
  </div>
  ...
  <div class="border-t border-slate-100 pt-3 text-sm">
    <div class="flex justify-between"><span>Subtotal</span><span>$${(sale.subtotal || 0).toFixed(2)}</span></div>
    <div class="flex justify-between"><span>Descuento</span><span>-$${(sale.discount || 0).toFixed(2)}</span></div>
    <div class="flex justify-between font-bold text-orange-600"><span>Total</span><span>$${(sale.total || 0).toFixed(2)}</span></div>
  </div>

  <button onclick="ReportsPage.printSale(${sale.id})" class="w-full py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-lg">
    Imprimir Ticket
  </button>
</div>
`,
