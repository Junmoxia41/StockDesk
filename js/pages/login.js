/**
 * Login Page - Admin + Cajero con contraseña (cliente)
 */
const LoginPage = {
  state: {
  ...
  <div class="w-full max-w-5xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[600px] animate-scale-in">
    ...
    <div class="md:w-5/12 bg-gradient-to-br from-slate-800 to-slate-900 p-8 flex flex-col justify-between text-white relative overflow-hidden">
      ...
      <div class="relative z-10">
        ...
        <h2 class="text-3xl font-bold leading-tight mb-4">${this.getLeftTitle()}</h2>
        <p class="text-slate-400">${this.getLeftDescription()}</p>
      </div>

      <div class="relative z-10 text-xs text-slate-500">System v2026.4 • Local Mode</div>
    </div>
...
`;
  ...
  getLeftDescription() {
    switch (this.state.view) {
      case 'selection':
        return 'Selecciona tu perfil de acceso para comenzar.';
      case 'admin-login':
        return 'Acceso para propietario/gerencia. Configuración, inventario y reportes.';
      case 'admin-register':
        return 'Crea un administrador con privilegios totales.';
      case 'cashier-select':
        return 'Selecciona tu usuario y entra con contraseña.';
      default:
        return 'Plataforma integral de gestión.';
    }
  },
...
<div class="space-y-6 animate-fade-in-up">
  ...
  <button onclick="LoginPage.setView('cashier-select')"
    ...
    <div>
      ...
      <p class="text-sm text-slate-500 mt-1">Usuarios registrados por el Admin.</p>
    </div>
  </button>
</div>
`;
...
<div class="animate-fade-in-up">
  ...
  <form onsubmit="LoginPage.handleAdminLogin(event)" class="space-y-5">
    ...
    <div>
      ...
      <input type="text" id="admin-user"
        class="w-full px-4 py-3.5 rounded-xl border border-slate-200 focus:border-orange-500 outline-none transition bg-slate-50 focus:bg-white"
    ...
    <button type="submit"
      class="w-full py-4 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl shadow-lg shadow-orange-500/30 transition transform hover:scale-[1.02]">
      Entrar al Sistema
    </button>
  </form>
  ...
  <p class="text-center text-xs text-slate-300 mt-8">
    Credencial por defecto: admin / admin (si el admin mantiene esa contraseña)
  </p>
</div>
`;
  },
  ...
  <form onsubmit="LoginPage.handleAdminRegister(event)" class="space-y-4">
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        ...
        <input type="password" id="reg-pass" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-orange-500 outline-none transition bg-slate-50">
      </div>
      <div>
        <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Confirmar</label>
        <input type="password" id="reg-confirm" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-orange-500 outline-none transition bg-slate-50">
      </div>
    </div>
...
<div class="animate-fade-in-up h-full flex flex-col">
  ...
  <p class="text-slate-500 mb-6">Ingresa con contraseña para iniciar turno de venta.</p>
  ...
  <div class="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar max-h-[400px]">
    ...
    <div class="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-300">
      ...
      <p class="text-xs text-slate-500 max-w-xs mx-auto">Ingresa como Administrador y ve a "Usuarios" para crear cuentas.</p>
    </div>
    `}
  </div>
</div>
`;
  ...
  // ====== Lockout local por intentos ======
  getAttemptState(username) {
    const attempts = Store.get('stockdesk_login_attempts') || {};
    return attempts[username] || { count: 0, lockedUntil: 0, lastAttempt: 0 };
  },

  saveAttemptState(username, state) {
    const attempts = Store.get('stockdesk_login_attempts') || {};
    attempts[username] = state;
    Store.set('stockdesk_login_attempts', attempts);
  },
  ...
  isLocked(username) {
    ...
    if (!sec.lockOnFailure) return false;
    const st = this.getAttemptState(username);
    return Date.now() < (st.lockedUntil || 0);
  },

  recordFail(username) {
    const sec = Store.security.get();
    if (!sec.lockOnFailure) return;

    const st = this.getAttemptState(username);
    st.count = (st.count || 0) + 1;
    st.lastAttempt = Date.now();

    // 5 intentos -> bloqueo 10 min (local)
    if (st.count >= 5) {
      st.lockedUntil = Date.now() + (10 * 60 * 1000);
    }

    this.saveAttemptState(username, st);
  },
  ...
  async handleAdminLogin(e) {
    ...
    if (!user || !pass) {
      Components.toast('Completa usuario y contraseña', 'warning');
      return;
    }

    if (this.isLocked(user)) {
      const st = this.getAttemptState(user);
      const mins = Math.ceil((st.lockedUntil - Date.now()) / 60000);
      Components.toast(`Usuario bloqueado. Intenta en ${mins} min.`, 'error', 4000);
      return;
    }

    const users = Store.get(Store.KEYS.USERS) || [];

    // Soporte a admin/admin:
    // - Si existe usuario "admin" y su password coincide con "admin", permite.
    // - Si no existe admin, crea uno con password "admin" (hash) y permite.
    if (user === 'admin' && pass === 'admin') {
      ...
      this.clearAttemptState(user);
      this.loginSuccess(adminUser);
      return;
    }

    const found = users.find(u => u.username === user && (u.role === 'Administrador' || u.role === 'Gerente'));
    if (!found) {
  ...
  promptCashierPassword(username) {
    const users = Store.get(Store.KEYS.USERS) || [];
...
<form class="space-y-3">
  <div>
    ...
    <input type="password" id="cashier-pass" class="w-full px-4 py-2.5 rounded-lg border border-slate-200" placeholder="••••••••">
    <p class="text-xs text-slate-500 mt-2">Si no recuerdas tu contraseña, contacta al administrador.</p>
  </div>
</form>
`,
      confirmText: 'Entrar',
      ...
      onConfirm: async () => {
        ...
        if (!found.password) {
          Components.toast('Este usuario no tiene contraseña configurada. Edítalo en Usuarios.', 'error');
          return false;
        }
        ...
        this.clearAttemptState(username);
        this.loginSuccess(found);
        return true;
      }
    });
